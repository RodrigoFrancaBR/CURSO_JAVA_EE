...
CREATE TABLE IF NOT EXISTS `db_curso`.`TB_UNIDADE` (
  ...
  `status` TINYINT NOT NULL DEFAULT 1 COMMENT 'USADO PARA EXCLUSÃO LÓGICA - 0 desativada 1 ativa',
  PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `db_curso`.`TB_ALUNO` (
  ...
  `pai` VARCHAR(40) NULL,
  `mae` VARCHAR(40) NULL,
  `sit_aluno` TINYINT NOT NULL DEFAULT 1 COMMENT 'USADO PARA EXCLUSÃO LÓGICA - 0 desativado 1 ativo',
  PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `db_curso`.`TB_PARCELA` (
  ...
  `sit_parc` TINYINT NOT NULL COMMENT '1= PAGO, 2 = A VENCER, 3 = ATRASADO 4 = CANCELADO POR ACORDO',
  PRIMARY KEY (`id`),
